...
function create_new_vm_with_invoice($vars) {
    ...
    $username = $userId; // XXX cleanup once username is removed
            ...
            if ($products) {
                foreach ($products as $product) {
                    ...
                    if (stristr($product['name'], "storage"))
                        $vmData['diskspace'] = $product['count'] * $amount * 1024;
                    // OMS expects values in MB
    ...
    addCreditForUserId($userId, $amountPaid, $desc);
 ...
 */
function addCreditForUserId($userId, $amount, $desc) {
    if ($amount < 0) {
    ...
    if ($clientData['result'] == "success") {
        logActivity("Successfully added amount of " . $amount . " credit for userId:" . $userId);
        return true;
