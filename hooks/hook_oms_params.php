...
function addParams($vars) {
    ...
    $arr = array("oms_img" => $oms_img, "oms_link" => $oms_hostname,
                 "oms_iframe_logout" => $oms_iframe_logout,
                 "OMS_BUNDLE_ID" => $oms_bundles_group_id,
                 "OMS_GENERATED_ID" => $oms_generated_group_id);
...
function addIframe($vars) {
    ...
    if ($_COOKIE[p]) {
        ...
        if ($userid) {
            ...
            //return '';
            $oms_iframe_src = $oms_hostname . 'basicauth?username=' . $userid . '&password=' . $pass;
